...
case $choice in
    1)
        ...
        # Инициализация ноды
        gaianet init --config --chat-url https://huggingface.co/gaianet/gemma-2-9b-it-GGUF/resolve/main/gemma-2-9b-it-Q5_K_M.gguf --prompt-template gemma-instruct  
    ...
    2)
        ...
        curl -L https://raw.githubusercontent.com/noxuspace/cryptofortochka/main/gaia_bot.py -o gaia_bot.py

        # Запрос адреса ноды
        echo -e "${YELLOW}Введите адрес вашей ноды${NC}"
        read -p "Адрес ноды: " NODE_ID
        
        # Подставляем введенный адрес в файл gaia_bot.py
